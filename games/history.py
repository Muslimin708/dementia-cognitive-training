...
    """Convert a database timestamp into a readable value."""
    if value is None:
...
        result["total_attempts"] for result in results
    ) / session_count
    best_result = max(
        results,
        key=lambda result: (
...
    )
    summary_df["Average Score"] = summary_df[
        "Average Score"
    ].round(1)
    summary_df["Average Accuracy"] = summary_df[
        "Average Accuracy"
    ].round(1)
    summary_df["Average Attempts"] = summary_df[
        "Average Attempts"
    ].round(1)
    st.markdown("#### Performance by Game")
    chart_data = summary_df[
        ["Game", "Average Accuracy"]
    ].set_index("Game")
    st.bar_chart(
        chart_data,
...
        results,
        key=lambda result: (
            str(result["completed_at"] or ""),
            int(result["result_id"] or 0),
        ),
        reverse=True,
    )
...
            )
            st.caption(
                f"Completed: {result['completed_display']}"
            )
